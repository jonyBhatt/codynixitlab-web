  ...
  return (
    ...
    >
      <div className="max-w-7xl  mx-auto px-6 flex justify-between items-center">
          ...
          {theme === "light" ? (
            <>
              <img
                src={logo}
                alt="Codynix IT Lab"
                className="sm:h-20 h-10 w-auto rounded-lg"
              />
            </>
          ) : (
            <>
        ...
        >
          <div
            className={`transition-all duration-500 transform ${isMobileMenuOpen ? "rotate-90 scale-110" : "rotate-0 scale-100"}`}
          >
            {isMobileMenuOpen ? (
              <X
                size={28}
      ...
      {/* Mobile Nav Menu */}
      <div
        className={`md:hidden absolute top-full left-0 right-0 bg-background shadow-2xl border-t border-border/50 p-8 flex flex-col gap-6 transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] origin-top transform ${isMobileMenuOpen ? "translate-y-0 opacity-100 visible" : "-translate-y-4 opacity-0 invisible"}`}
      >
        ...
        <ModeToggle />
        <div
          className={`pt-4 border-t border-border/50 transition-all duration-500 transform ${isMobileMenuOpen ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"}`}
